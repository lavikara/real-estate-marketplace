...
contract Ownable {
    ...
    modifier onlyowner() {
        require(msg.sender == _owner, "Ownable: caller is not contract owner");
        _;
    }

    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event TransferedOwnership(address indexed previousOwner, address indexed newOwner);
    ...
    function transferOwnership(address newOwner) public onlyowner {
        ...
        require(newOwner != address(0), "Ownable: the address should be a real address");
        _owner = newOwner;
...
contract Pausable is Ownable {
    ...
    modifier paused() {
        require(_paused == true, "Pausable: contract is currently not paused");
        _;
    }
    ...
    function pause (bool _status) onlyowner public {
        require(_status != _paused, "Pausable: new status is the same as paused");
        _paused = _status;
        if(_status) {
...
contract ERC721 is Pausable, ERC165 {
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        require(owner != address(0), "ERC721: invalid account");
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != tokenOwner, "ERC721: can not approve to same account");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == tokenOwner || isApprovedForAll(tokenOwner, msg.sender), "ERC721: this address cant approve transfer");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(_exists(tokenId) == false, "ERC:721: token id already exist");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_isApprovedOrOwner(from, tokenId), "ERC721: this address is not the token owner");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "ERC721: address is invalid");
